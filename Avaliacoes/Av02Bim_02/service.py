...
class Service:
    @staticmethod
    def cliente_inserir(id, nome, email, fone, data_ultima_compra): # 5
        obj = Cliente(id, nome, email, fone, data_ultima_compra)
    ...
    @staticmethod
    def cliente_atualizar(id, nome, email, fone, data_ultima_compra): # 5
        obj = Cliente(id, nome, email, fone, data_ultima_compra)
    ...
    def clientes_sem_compras_recentes():
        ...
        for aux in Service.cliente_listar(): # 3
            if aux.get_data_ultima_compra() < (datetime.now() - sm): # 3
                r.append(aux) # 3
        return r # 3
